...
EOF
	[[ "$GET" == "1" ]] && cat <<EOF >> $FULL_FILE.cpp

void $FILE_NAME::get(const fleropp::io::RequestData& request) {

}
EOF

	[[ "$POST" == "1" ]] && cat <<EOF >> $FULL_FILE.cpp

void $FILE_NAME::post(const fleropp::io::RequestData& request) {

}
EOF
	[[ "$PUT" == "1" ]] && cat <<EOF >> $FULL_FILE.cpp

void $FILE_NAME::put(const fleropp::io::RequestData& request) {

}
EOF
	[[ "$DELETE" == "1" ]] && cat <<EOF >> $FULL_FILE.cpp

void $FILE_NAME::del(const fleropp::io::RequestData& request) {

}
EOF

	cat <<EOF > $FULL_FILE.hpp
